...
export class RecordsService {
  ...
  addRecord(record: IRecord): Observable<IRecord> {
    ...
    let params = new HttpParams()
      .set('TitleRecord', record.titleRecord)
      .set('YearOfPublication', record.yearOfPublication?.toString() || '')
      .set('Price', record.price.toString())
      .set('Stock', record.stock.toString())
      .set('Discontinued', record.discontinued ? 'true' : 'false');

    // Make sure groupId has a value before adding it
    if (record.groupId) {
      params = params.set('GroupId', record.groupId.toString());
    }
    ...
    // Get headers with the authentication token
    const headers = this.getHeaders();
    
    // If there is a photo, send as multipart/form-data, otherwise send null
